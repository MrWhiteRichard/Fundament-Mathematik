...
\begin{enumerate}[label = \textbf{\alph*)}]
  ...
  \begin{align*}
    ...
    \end{pmatrix}.
  \end{align*}
  Dieses Problem ist nun eindeutig lösbar und wir müssen ein $b \in \R$ finden,
  sodass $y^{\prime}(1) = 0$.
  Irgendwie müssen wir jetzt verwenden, dass die Abbildung
  \begin{align*}
    T_t(y) := \frac{F(t,y)}{a}
  \end{align*}
  eine Kontraktion ist und mit dem Banachschen Fixpunktsatz für jedes $t \in [0,1]$
  ein eindeutiges $y_t \in \R$ exisitiert mit $T_t(y_t) = y_t$. \\
  Definiere $y(t) = y_t$. Dann gilt:
  \begin{align*}
    ay(t) - F(t,y) \equiv 0
  \end{align*}
  ...
  \begin{align*}
    y^{\prime}(1) = \int_0^1 ay - F(t,y(t)) dt
  \end{align*}
  Definiere die Abbildung
  \begin{align*}
    A := \{x \in C([0,1];\R): \forall t \in [0,1]: \|x(t)\| \leq b \}
  \end{align*}
  \begin{align*}
    T(x)(t) := \int_0^t ay - F(t,x(t)) dt
  \end{align*}
