...
\begin{enumerate}[label = \textbf{\alph*)}]
  ...
  \begin{enumerate}[label = \textit{\roman*)}]
    ...
    \begin{enumerate}[label = \textit{\arabic*.}]
      ...
      \end{align}
      Wir verwenden nun die Newton Darstellung des Interpolationspolynoms über die dividierten Differenzen, wobei für $j=1,...,r-1$ gilt $y[z_j] = p^{\prime}(x_{j+1}) = 0$ und somit aufgrund der rekursiven Definition der d. D. auch $y[z_1,...,z_j]=0, j = 1,\dots,r-1$ und $y[z_1,...,z_r] = \frac{1}{C}p^{\prime}(z_r)$
