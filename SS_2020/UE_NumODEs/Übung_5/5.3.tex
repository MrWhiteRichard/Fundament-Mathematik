...
\begin{enumerate}[label = \textbf{\alph*)}]
  ...
  \begin{enumerate}[label = \textit{\roman*)}]
    ...
    \begin{enumerate}[label = \textit{\arabic*.}]
      ...
      Umkehrung trivial.
      \item Fall ($k>1$):
      Definiere das Polynom $p_0 \in \Pi_r$ durch
      \begin{align*}
        p_0(x) := \sum_{i=0}^{r}a_{i}x^i \text{~mit~} &a_i = 0, i=k,...r, \\
        &a_i = 1, i=1,...,min(r,k-1), \\
        &a_0 = - \sum_{i=1}^{r}a_{i}x_0^{i}
      \end{align*}
      Nun gilt $p_0^{(k)}(x) = \sum_{i=k}^{r} \frac{i!}{(i-k)!}a_{i}x^{i-k} = 0$, insbesondere $p_0^{(k)}(x_j) = 0, j=1,...,r$. Außerdem ist durch Definition von $a_0$ auch $p_0(x_0) = a_{0}x_0^{0} + \sum_{i=1}^{r}a_{i}x_0^i = 0$.
  ...
  \begin{enumerate}[label = \textit{\roman*)}]
    ...
    \begin{enumerate}[label = \textit{\arabic*.}]
      ...
      \item Fall ($k>1$):
      (Fast) analog zu \textbf{c)} definiert man das Polynom $p_0 \in \Pi_r$ (o.B.d.A. $r>1$) durch
      \begin{align*}
        p_0(x) := \sum_{i=0}^{r}a_{i}x^i \text{~mit~} &a_i = 0, i=k,...r, \\
        &a_i = 1, i=2,...,min(r,k-1), \\
        &a_0 = - \sum_{i=1}^{r}a_{i}x_0^{i} \\
        &a_1 = - \frac{a_0 + \sum_{i=2}^{r}a_{i}x_1^{i}}{x_1}
      \end{align*}
      Aufpassen: $a_0$ und $a_1$ so nur implizit definiert, aber eindeutig lösbar (da $x_0 \neq x_1$).
      Wieder gilt $p_0^{(k)}(x) = \sum_{i=k}^{r} \frac{i!}{(i-k)!}a_{i}x^{i-k} = 0$, insbesondere $p_0^{(k)}(x_j) = 0, j=2,...,r$. Außerdem ist durch Definition von $a_0$ und $a_1$ auch $p_0(x_0) = a_{0}x_0^{0} + \sum_{i=1}^{r}a_{i}x_0^i = 0$ und $p_0(x_1) = a_{0}x_1^{0} + a_{1}x_1^{1} + \sum_{i=2}^{r}a_{i}x_1^i = 0$.
      Somit gilt $\|p_0\|_k = 0$ aber $p_0 \neq 0$. Also ist $\|\cdot\|_k$ für $k>1$ keine Norm.
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
