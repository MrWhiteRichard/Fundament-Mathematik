...
\end{align*}
Also gilt auch $\sup_{x \in \Omega}  \vbraces{\psi_l} \leq \sup_{x \in \Omega} \vbraces{D^\alpha \phi(x)} < \infty$ womit wir sehen, dass der linke Grenzwert für $l \to \infty$ existiert. Klarerweise gilt für alle $l \in \N$, dass $\supp\psi_l \subseteq \supp \phi$. Wegen der lokalen Endlichkeit (hineinziehen des Differentialoperators) und der eben gezeigten Konvergenz gilt
\begin{align*}
\lim_{l \to \infty} \sup_{x\in\Omega} \vbraces{D^\alpha \pbraces{\phi(x) - \psi_l(x)}} = \lim_{l \to \infty} \sup_{x\in\Omega} \vbraces{D^\alpha \sum_{k=l}^\infty \zeta_k(x)\phi(x)} = \lim_{l \to \infty} \sup_{x\in\Omega} \vbraces{\sum_{k=l+1}^\infty \zeta_k(x)D^\alpha \phi(x)} = 0.
\end{align*}
Die Folge von Fuktionen $\psi_l$ konvergiert also im Sinne von Definition 3.2. Damit und mit der Stetigkeit von $T$ im Sinne der eben gezeigten Konvergenz gilt 
\begin{align}
