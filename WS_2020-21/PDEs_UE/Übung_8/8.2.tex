...
\begin{enumerate}[label = (\alph*)]
  ...
  \item Aus der Definition von $H_0^2(\Omega)$ folgt, dass wir $v \in H_0^2(\Omega)$,
    dass wir $v$ darstellen können als
    \begin{align*}
    v = \lim_{n \to \infty}v_n \text{ in } H^2(\Omega), \quad (v_n)_{n\in\N} \subset C_0^{\infty}(\Omega)
    \end{align*}
    Es gilt also
    \begin{align*}
      \|v-v_n\|_{H^2(\Omega)} = \left(\sum_{|\alpha|\leq 2}\int_\Omega (D^\alpha(v-v_n))^2dx\right)^{1/2}
      \xrightarrow{n \to \infty} 0.
    \end{align*}
    Dami folgt aber auch
    \begin{align*}
      \|\nabla v - \nabla v_n\|_{H^1(\Omega)} = \|\nabla(v-v_n)\|_{H^1(\Omega)} &= \left(\sum_{|\alpha|\leq 1}\int_\Omega (D^\alpha\nabla(v-v_n))^2dx\right)^{1/2} \\
      &\leq \left(\sum_{|\alpha|\leq 2}\int_\Omega (D^\alpha(v-v_n))^2dx\right)^{1/2}
      = \|v-v_n\|_{H^2(\Omega)}
      \xrightarrow{n \to \infty} 0.
    \end{align*}
  Also gilt auch $\nabla v \in \overline{C_0^{\infty}(\Omega)}^{\|\cdot\|_{H^1(\Omega)}} = H_0^1(\Omega)$ und daher $T(\nabla v) = T(v) = 0$. \\
  ...
  \begin{itemize}
    \item Stetigkeit: ($|\Hess u|$ bezeichnet die Frobenius-Norm der Hesse-Matrix.)
