...
%				bzw $g(n_1,\dots,n_k,z) := \mu y \leq z(R(n_1,\dots,n_k,y))$. \\
				...
				Mit der oberen Schranke $\varphi_{prim}(n+1) \leq 2^{2^n}$ (Induktion und die Tatsache $p_{n + 1} \leq p_1\cdots p_n + 1$) erhalten wir für
				\begin{align*}
					g(m,z) := h_R(m,z)
				\end{align*}
				Die primitiv rekursive Funktion 
				\begin{align*}
				\varphi_{prim}(0) := 1, \quad \varphi_{prim}(n + 1) := 2\chi_{\{0\}}(n) + (1 - \chi_{\{0\}}(n)) g(\varphi_{prim}(n), 2^{2^n})
				\end{align*}		
...
%				\end{align*}
				...
				\begin{align*}
				S := \Bbraces{(n,k,y) \in \N^3 \mid \mathrm{div}(n, \varphi_{prim}(k)^y) = 0}
				\end{align*}
				Jetzt können wir $(n,k) \mapsto (n)_k$ primitv rekursiv beschreiben durch
				\begin{align*}
					(n)_k = h_S(n,k,n) - 1. %[\mu y \leq n(\mathrm{div}(n,\varphi_{prim}(k)^y) = 0)] - 1.
				\end{align*}
		\end{enumerate}
