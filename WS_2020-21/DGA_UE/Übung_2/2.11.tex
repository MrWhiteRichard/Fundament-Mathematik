...
Seien $V$ die Ecken (\enquote{vertices}) und $E$ die Kanten (\enquote{edges}) von $G$.
Offensichtlich ist $|V| = 6$.
Jede Ecke ist mit jeder anderen ($5$ Stück) verbunden.
Wenn wir ein Eck wegnehmen und seine Kanten \enquote{ernten} (vgl. \enquote{Blätter} $\rightsquigarrow$ \enquote{Früchte}?), dann verlieren die übrigen Ecken jeweils eine Kante.
...
Sei $f$ eine von $2^{15}$ beliebigen \enquote{Färbungs-Funktionen}.
