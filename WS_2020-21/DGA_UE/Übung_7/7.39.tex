...
\begin{enumerate}[label = (\Alph*)]
    ...
    Das ist äquivalent dazu, dass $B$ genau eine Zusammenhangskomponente hat.
    Sei $i = 1, \dots, n$ und verbinde $e_{i+1}$ zwei Zusammenhangskomponenten $Z_1, Z_2$ von $(V, E_i)$.
    Dann gilt auf jeden Fall $e_{i+1} \in E_{i+1}$.
    Wenn es keinen Zyklus in $(V, E_i \cup \Bbraces{e_{i+1}})$ gibt, ist das klar, weil dann ja $E_{i+1} = E_i \cup \Bbraces{e_{i+1}}$.
    Wenn es aber doch einen Zyklus gibt, dann muss dieser vollständig in einer Zusammenhangskomponente $Z_1$ oder $Z_2$ liegen.
    Wenn $e_{i+1}$ zwei Zusammenhangskomponenten verbindet wird $e_{i+1}$ vom Algorithmus also nicht entfernt.
    ...
    Wenn eine ($e_i$) verwendet wurde, dann wurde sie laut den oberen Überlegungen nicht entfernt.
