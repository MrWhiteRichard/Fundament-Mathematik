...
\begin{solution}
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\State Sei \textit{bekannt} ein neues Datenfeld der Länge $|V|$ überall mit \textbf{faslch} initialisiert
			\State Sei $M$ neue Warteschlange leer initialisiert
			\State $\textsc{Hinzufügen}(M,s)$
			\State $\textit{bekannt}[s] := \textbf{wahr}$
