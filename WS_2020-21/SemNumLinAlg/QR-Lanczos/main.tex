...
\begin{theorem}[Konvergenz der Eigenwerte hermitescher Matrizen]
	...
	\begin{equation}
		\begin{aligned}
			...
			\lambda_i^{(m)} &= \max_{\substack{v\in \C^m\setminus \{0\} \\ (u_j^{(m)},v) = 0, j = 1,\dots,i-1}} \frac{(T_m v,v)}{(v,v)} \\
			&= \max_{\substack{v\in \C^m\setminus \{0\} \\ (u_j^{(m)},v) = 0, j = 1,\dots,i-1}} \frac{(AVv,Vv)}{(Vv,Vv)}\\
			&= \max_{\substack{w\in \mathcal{K}_m\setminus \{0\} \\ (Vu_j^{(m)},w) = 0 j = 1,\dots,i-1}} \frac{(Aw,w)}{(w,w)}
		\end{aligned}
	\end{equation}

	Da die $u_j^{(m)}, j = 1,\dots,m$ eine Orthonormalbasis des $\C^m$ aus Eigenvektoren von $T_m$ sind, sind somit die $\tilde{u}_j^{(m)} \coloneqq \nicefrac{Vu_j^{(m)}}{\norm{Vu_j^{(m)}}}$ eine Orthonormalbasis von $\mathcal{K}_m$. Sei nun $w\in \mathcal{K}_m$ gegeben durch $w = \sum_{k=1}^{m} p(\lambda_k^{(m)}) \beta_k \tilde{u}_k^{(m)}$. Diese Darstellung ergibt sich aus der Tatsache, dass $w$ als Linearkombination von $\tilde{u}_j^{(m)}, \quad j = 1,\dots, m$ geschrieben werden kann und Lemma \ref{v0_Polynom_Darstellung}. Dann gilt für alle $j < i$
	\begin{equation*}
...
\begin{figure}[H]\label{error_eigv_laplace}
...
\lstset{caption={Implementation der QR-Zerlegung für Hessenbergmatrizen mit Givens-Rotationen}}
...
\lstset{caption={Implementation eines Schrittes des QR-Verfahrens für Hessenbergmatrizen}}
\begin{lstlisting}
...
\lstset{caption={Implementation des QR-Verfahrens mit Wilkinson-Shift für Hessenbergmatrizen}}
