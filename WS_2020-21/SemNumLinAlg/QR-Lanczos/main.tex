...
Die Konvergenz dieses Algoritmus kann unter gewissen Voraussetzungen gezeigt werden. Wir Zitieren hier nur den Satz und verweisen für den Beweis auf [1], Satz 9.15.
%\cite{Nannen-Skript}, Satz 9.15.

\begin{theorem}[Konvergenz des QR-Verfahrens]
	\label{Konv_QR}
	Sei $A \in \C^{n\times n}$ eine diagonalisierbare Matrix mit Eigenwerten $\lambda_1,\dots,\lambda_n \in \C$ und $|\lambda_1| > |\lambda_2| > \dots > |\lambda_n| > 0$. Weiters sei $V \in \C^{n \times n}$ die Matrix, die die Eigenvektoren $v_1,\dots v_n$ zu den jeweiligen Eignwerten als Spalten enthält. Zudem existiert eine untere normierte Dreiecksmatrix $L$ und eine obere Dreiecksmatrix $U$, sodass $V^{-1} = LU$.\\
...
Wir betrachten die Matrizen $M^{(t)} \coloneqq \Lambda^t L \Lambda^{-t},\, t \in \N$, die im Beweis von Satz \ref{Konv_QR} vorkommt. Diese haben die Form
...
Eine weitere Beschleunigung erreichen wir bei der QR-Zerlegung, wenn die Matrix $A$ Hessenbergform hat. Auf diesen Fall werden wir jetzt im nächsten Anschhnitt eingehen.
...
In diesem Abschnitt beweisen wir, dass die durch das Lanczos-Verfahren erhaltenen Eigenwerte $\lambda_1^{(m)},\dots,\lambda_m^{(m)}$ gegen Eigenwerte $\lambda_1,\dots,\lambda_m$ der hermiteschen Matrix $A \in \C^{n \times n}$ konvergieren.
...
\begin{theorem}[Konvergenz der Eigenwerte hermitscher Matrizen]
	...
	\begin{equation}
		...
		\begin{aligned}
			...
			&\le \max_{\substack{T \subseteq \mathcal{K}_m\\ \exists S:\, V(S)=T \\ \dim S = k}} \min_{w\in S\setminus\{0\}} \frac{(Aw,w)}{(w,w)}\\
	...
	Wir bezeichnen mit $\mathcal{P}_{u_i}, \, i = 1,\dots, m-1$ die Projektion auf den eindimensionalen Unterraum, der vom Vektor $u_i$ aufgespannt wird. Damit gilt
		\begin{equation*}
