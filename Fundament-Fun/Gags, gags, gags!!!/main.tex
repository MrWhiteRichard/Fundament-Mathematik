\documentclass{article}

\input{../../Fundament-LaTeX/packages.tex}
\input{../../Fundament-LaTeX/macros.tex}
\input{../../Fundament-LaTeX/environments.tex}

\newtheorem{gag}{Scherz}[section]

\parindent 0pt

\title
{
  Gags, gags, gags!!! \\
  \vspace{4pt}
  \normalsize
  \textit{Hierhin kommen alle Späße die uns einfallen!}
}
\author{}
\date{}

\begin{document}

\maketitle

\section{Apples}

\begin{gag}

  \href{https://knowyourphrase.com/how-do-you-like-them-apples}{Zitat} aus \Quote{Good Will Hunting}: \\

  Hunting:
  \Quote{Do you like apples?}

  Rival:
  \Quote{Yeah.}

  Hunting:
  \Quote{Well, I got her number. How do you like them apples?}

\end{gag}

\begin{gag}

  Ein \textit{Spindeltorus} entsteht durch Rotation des Kreissegmentes

  \begin{align*}
    x & = (R + r \cos{\theta}) \cos{\varphi} \\
    y & = (R + r \cos{\theta}) \sin{\varphi} \\
    z & = r \sin{\theta}
  \end{align*}

  um die $z$-Achse für $0 < R < r$, $R + r \cos{\theta} \geq 0$.

  \begin{figure}[H]
    \centering
    \subfloat[Paul Winkler]{
      \includegraphics[width = 0.25 \textwidth]{images/apfel}
    }
    \subfloat[Michael Kaltenbäck]{
      \includegraphics[width = 0.25 \textwidth]{images/apple}
    }
    \hspace{0mm}
    \caption{Der Spindeltorus als Apfel}
    \label{fig:apfel_apple}
  \end{figure}

\end{gag}

\section{xkcd's}

\begin{figure}[H]
  \centering
  \subfloat[GIT]
  {
    \includegraphics[width = 0.45 \textwidth]{images/git_2x.png}
  }
  \subfloat[Depth-, Breadth-, \dots]
  {
    \includegraphics[width = 0.45 \textwidth]{images/depth_and_breadth2.png}
  }
  \hspace{0mm}
  \caption{xkcd's}
  \label{fig:xkcd}
\end{figure}

\section{MISC}

\begin{theorem}

  \begin{align*}
    B(0, 1)
    =
    \text{Einheits-Kreis}
    \approx
    \text{Einheits-Greis}
    \approx
    \text{Einheiz-Greis}
  \end{align*}

\end{theorem}

\begin{theorem}

  \Quote{A Kuh is a Vieh vom Mü-Viertl}, d.h.

  \begin{align}
    aq = a \phi \left( \frac{\mu}{4} \right)
  \end{align}

\end{theorem}

\begin{proof}
  Triviale Übungsaufgabe!
\end{proof}

\end{document}
