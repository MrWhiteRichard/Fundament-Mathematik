...
paste("100(1-0.05)% CI = [",x_1 - x_2 - z_alphah*sqrt((s_1+s_2)/n), ","
      ,x_1 - x_2 + z_alphah*sqrt((s_1+s_2)/n) , "]")
```
...
```{r}
z = (x_1-x_2)/sqrt((s_1+s_2)/n)

paste("p-value = ", 2*pnorm(-abs(z)))
```
Since the p-value is high we will favor the null instead of the alternative hypothesis.
...
```
This p-value is still rather large and again supports the null, which is still the same as in (b).

(d)
```{r}
z_new = (x_1-x_2 - 25)/sqrt((s_1+s_2)/n)

paste("p-value = ", 2*pnorm(-abs(z_new)))
```
Here we see that the p-value is, unsurprisingly, very small so we will reject the null in favor of the alternative.
