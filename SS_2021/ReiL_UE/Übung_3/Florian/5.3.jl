...
function sarsa(gridworld, max_steps, alpha, epsilon)
    Q = Dict([state,action] => 0.0 for state in gridworld.states, action in gridworld.actions)
    policy = Dict(state => gridworld.actions for state in gridworld.states)
    ...
    while total_episode_length < max_steps
        episode_counter += 1
        Q, policy, episode_length = generate_episode(Q, policy, gridworld, alpha, epsilon)
...
function generate_episode(Q, policy, gridworld, alpha, epsilon)
    current_state = gridworld.start
    if rand() < epsilon
        current_action = rand(gridworld.actions)
    else
        current_action = rand(policy[current_state])
    end
    current_wind = 0
    episode_length = 1
    while current_state != gridworld.goal
        episode_length += 1
        current_wind = rand(gridworld.wind[current_state[1]])
        ...
        if rand() < epsilon
            new_action = rand(gridworld.actions)
        else
        ...
        max_Qvalue = maximum([Q[[current_state, action]] for action in gridworld.actions])
    ...
    return Q, policy, episode_length
end
...
for (actions,col) in zip((four_actions, eight_actions, nine_actions),("red", "blue", "green"))
    ...
    gridworld = GridWorld(grid, actions, deterministic_wind, start, goal)

    println("Deterministic Wind, ", string(length(actions)) * " Actions")
    Q, policy, episode_length_cumsum = sarsa(gridworld, 20000, alpha, epsilon)
    ...
    gridworld = GridWorld(grid, actions, stochastic_wind, start, goal)

    println("Stochastic Wind, ", string(length(actions)) * " Actions")
    Q, policy, episode_length_cumsum = sarsa(gridworld, 100000, alpha, epsilon)
