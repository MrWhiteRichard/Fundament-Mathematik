...
eqlatex=string(a)+"$x^4y'''+"+string(b)+"x^3y''+"+string(c)+"x^2y'+"+string(d)+"xy=0$";
...
hnum3 = plot(xs,ys3, 'g');
% proof
lhs=a*xs.^4.*ys3 + b*xs.^3.*ys2 + c*xs.^2.*ys1 + d*xs.*ys;
...
ylabel('$y(x)$')
xlim([x1 x2])
htit=title(join(eqlatex),'fontsize',16);
yl=get(gca,'ylim');
set(htit,'position',get(htit,'position')+[0 0.01*diff(yl) 0]);
legend([hnum,hnum1,hnum2,hnum3,hproof],["Numerical Solution", "First derivative", "Second derivative", "Third derivative", "Left-hand side"], 'Location', 'northwest');
