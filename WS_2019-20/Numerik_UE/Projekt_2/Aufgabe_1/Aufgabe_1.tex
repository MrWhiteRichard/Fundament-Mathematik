...
&2: r^{(0)} = b - Ax^{(0)} &\\
&3: d^{(0)} = r^{(0)} &\\
&4: \textbf{while}~ \vbraces{r^{(t)}} > \textbf{TOL} do &\\
...
\FloatBarrier
\subsection*{c)}
Um die Effizienz weiter zu steigern, müssen wir natürlich noch irgendwie die dünne Besetzheit ausnutzen.
...
Man sieht außerdem, dass beide Algorithmen eine Laufzeit von etwa $\Landau{n^2}$ haben.
\subsection*{e)}
...
postiv definiten Zufallsmatrizen. Der Zeitgewinn schlägt sich leider nicht ganz so deutlich wieder,
