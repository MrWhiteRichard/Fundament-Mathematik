...
def zufallsmatrix(z,n):
    ...
    A = A + A.T + np.diag(np.random.rand(n)*3000)
    return A
...
x = np.array([i for i in range(500,n+1,100)])
time_np = []
time_sparse = []
...
# tol = 10**(-8)
#
# x = np.array([i for i in range(500,n+1,500)])
# time_np = []
# time_cg = []
#
# for i in x:
...
#     A_sparse = Sparse(False,A_part)
#     start = time.process_time()
#     c=Scg(A_sparse,b,x_0,tol)
#     end = time.process_time()
#     diff=end-start
#     time_cg.append(diff)
#     print(i)
#
# plt.loglog(x,time_cg,'-',label="cg-Verfahren (Sparse)")
# plt.loglog(x,time_np,':',label = "np.linalg.solve")
# plt.xlabel("Matrix-Dimension")
# plt.ylabel("Sekunden")
# plt.legend()
# plt.show()
